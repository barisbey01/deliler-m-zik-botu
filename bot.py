...
greeting = """
     DELİLER MÜZİK BOTUNA HOŞGELDİNİZ🙋😇
     BİZ TERCİH ETTİĞİNİZ İÇİN TEŞEKKÜRLER😌🖤
     YAPIMCI : @Barisbey01
...
not_found = """
SONUÇ BULUNAMADI TEKRAR DENEYİNİZ:/
"""
...
async def add_track(chat, audio):
    ...
    if "title" not in audio:
        await chat.send_text("sonuç bulunamadı tekrar dene")
        return
...
async def start(chat, match):
    ...
    await chat.send_text(selamın aleyküm)
...
async def stop(chat, match):
    ...
    await chat.send_text("ALLAHA EMANET OL")
